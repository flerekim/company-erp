...
## 2024-05-29 진행사항: 변경계약 등록 및 프로젝트 자동입력 개선

### 주요 개선 내역

1. **프로젝트 선택 UX 개선**
   - 프로젝트 텍스트 필드에서 입력 후 드롭다운에서 선택 시, 필드가 공란이 되는 문제 해결
   - 드롭다운에서 프로젝트를 선택하면 항상 텍스트 필드에 프로젝트명이 표시됨

2. **자동입력 로직 강화**
   - 프로젝트 선택 시 고객사명, 고객사 유형, 계약일, 처리방식, 정화방법, 검증업체, 담당자 등 관련 필드 자동 입력
   - 첫 번째 수주 정보가 있으면 해당 정보를 기반으로 자동 채움
   - 오염 정보(contamination_info)는 변경계약 시 새로 입력하도록 자동입력에서 제외

3. **타입 및 폼 스키마 정비**
   - OrderFormData 및 orderSchema에 project_name 필드 추가
   - 타입 에러 및 폼 유효성 검사 오류 해결

4. **불필요한 신규 프로젝트 생성 옵션 제거**
   - 변경 모드에서 "새 프로젝트 등록" 옵션 노출 방지

5. **코드 리팩토링**
   - setProjectSearch(project.project_name) 호출 위치 명확화
   - useEffect 및 useCallback 의존성 정비

---

### 스크린샷 예시
- 프로젝트명 입력 및 선택 시, 텍스트 필드와 관련 정보가 자동으로 채워지는 모습
- 변경계약 등록 시 UX 개선된 폼

---

### TODO
- 오염 정보 자동입력 정책 확정 (현재는 변경계약 시 수동입력)
- 프로젝트/수주 데이터 동기화 추가 검증